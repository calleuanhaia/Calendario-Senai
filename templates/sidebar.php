<?php
    ...
    $tipo = $_SESSION['usuario_tipo']; // Supondo que retorne 'coordenador', 'professor' ou 'aluno'
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<body class="flex h-screen w-full selection:bg-brand-accent selection:text-white">
    ...
    <aside class="w-72 bg-brand-dark1/95 backdrop-blur-xl border-r border-brand-dark4/40 flex flex-col relative z-20 shadow-[10px_0_30px_rgba(0,0,0,0.5)] transition-all">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
            ...
            <p class="text-[10px] font-bold text-brand-dark5 uppercase tracking-widest mt-8 mb-4 px-2">Atualizar Contas</p>
            <?php if ($tipo === 'aluno'): ?>
            <a href="index.php?page=atualizar_alunos.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                <span class="font-medium text-sm">Atualizar Alunos</span>
            </a>
            <?php endif; ?>
            <?php if ($tipo === 'coordenacao'): ?>
            <a href="index.php?page=lista_alunos_cordenacao.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                <span class="font-medium text-sm">Atualizar Alunos</span>
            </a>
            <?php endif; ?>
            <?php if ($tipo === 'professor'): ?>
            <a href="index.php?page=atualizar_professores.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                <span class="font-medium text-sm">Atualizar Professores</span>
            </a>
            <?php endif; ?>
            <?php if ($tipo === 'coordenacao'): ?>
            <a href="index.php?page=lista_professores_cordenacao.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                <span class="font-medium text-sm">Atualizar Professores</span>
            </a>
            <?php endif; ?>
            <?php if ($tipo === 'coordenacao'): ?>
            <a href="index.php?page=atualizar_cordenacao.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                <span class="font-medium text-sm">Atualizar Coordenação</span>
            </a>
            <?php endif; ?>

            <?php if ($tipo === 'coordenacao'): ?>
                <p class="text-[10px] font-bold text-brand-dark5 uppercase tracking-widest mt-8 mb-4 px-2">Deletar Contas (Acesso Restrito)</p>

                <a href="index.php?page=lista_alunos.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                    <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                    <span class="font-medium text-sm">Deletar Alunos</span>
                </a>

                <a href="index.php?page=lista_professores.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                    <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                    <span class="font-medium text-sm">Deletar Professores</span>
                </a>

                <a href="index.php?page=lista_cordenacao.php" class="flex items-center gap-3 px-4 py-3 text-brand-dark5 hover:text-white hover:bg-brand-dark2/50 rounded-xl transition-all group border-l-2 border-transparent hover:border-brand-dark5/30">
                    <i data-lucide="settings" class="w-5 h-5 group-hover:text-white transition-colors"></i>
                    <span class="font-medium text-sm">Deletar Coordenação</span>
                </a>
            <?php endif; ?>
        </nav>
        ...
        <div class="p-4 border-t border-brand-dark4/40 bg-brand-dark1">
            <div class="flex items-center gap-3 p-2 rounded-xl hover:bg-brand-dark2/50 transition-colors cursor-pointer">
                <div class="w-10 h-10 rounded-full bg-brand-dark3 flex items-center justify-center border border-brand-dark4">
                    <span class="font-bold text-sm text-brand-accentLight">
