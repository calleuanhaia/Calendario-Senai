...
<head>
    ...
    <title>SENAI Integra - Exclusão de Professores</title>
        ...
        <div class="bg-brand-dark1/90 backdrop-blur-2xl border border-brand-dark4/50 rounded-3xl p-6 sm:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.6)] relative overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-brand-dark2 rounded-xl flex items-center justify-center border border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.2)]">
                        <i data-lucide="user-minus" class="text-red-500 w-6 h-6"></i>
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold text-white mb-0.5">Exclusão de Professores</h1>
                        <p class="text-sm text-brand-dark5 font-light">Gerencie e remova os registros dos professores do sistema.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
                <div class="w-full relative">
                    ...
                    <input type="text" id="campoFiltro" class="bg-brand-dark2/50 border border-brand-dark4/60 text-white text-sm rounded-xl focus:ring-1 focus:ring-red-500 focus:border-red-500 block w-full pl-10 p-2.5 outline-none transition-all placeholder-brand-dark5" placeholder="Filtrar registro para exclusão...">
                </div>
            </div>
                    ...
                    <tbody id="corpoTabela" class="text-sm text-gray-300 divide-y divide-brand-dark4/30">
                        <?php if (count($professores) > 0): ?>
                            ...
                            <?php foreach ($professores as $professor): ?>
                                ...
                                <tr class="transition-colors duration-200 group hover:bg-red-500/5 hover:border-l-red-500 border-l-4 border-l-transparent linha-registro">
                                    <td class="py-4 px-6">
                                    ...
                                    <td class="py-4 px-6">
                                        <a href="api/deletar/delete.php?table=professores&id=<?php echo $professor['id']; ?>" 
                                           onclick="return confirm('Tem certeza que deseja excluir o professor <?php echo htmlspecialchars($nomeProfessor); ?>?');"
                                           class="flex items-center gap-1.5 text-red-400 hover:text-white transition-colors text-sm font-medium bg-red-500/10 hover:bg-red-600 px-3 py-1.5 rounded-lg border border-red-500/20 hover:border-red-500 w-max group-hover:shadow-[0_0_10px_rgba(239,68,68,0.3)]">
            ...
            <div class="mt-6 flex items-center justify-between text-xs text-brand-dark5">
                <p>Total de professores no sistema: <span class="font-bold text-white"><?php echo count($professores); ?></span><span id="contadorFiltro" class="font-medium text-red-400 ml-1"></span></p>
            </div>
