<?php
    ...
    $sql="SELECT * FROM cordenacao ORDER BY id DESC";
    $consulta=$pdo->query($sql);
    $cordenacoes=$consulta->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<head>
    ...
    <title>SENAI Integra - Exclusão de Coordenação</title>
        ...
        <div class="bg-brand-dark1/90 backdrop-blur-2xl border border-brand-dark4/50 rounded-3xl p-6 sm:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.6)] relative overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-brand-dark2 rounded-xl flex items-center justify-center border border-brand-accent/30 shadow-[0_0_15px_rgba(0,92,169,0.2)]">
                        <i data-lucide="shield-minus" class="text-brand-accent w-6 h-6"></i>
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold text-white mb-0.5">Remoção de Coordenação</h1>
                        <p class="text-sm text-brand-dark5 font-light">Gerencie e exclua os registros da coordenação do sistema.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
                <div class="w-full relative">
                    ...
                    <input type="text" id="campoFiltro" class="bg-brand-dark2/50 border border-brand-dark4/60 text-white text-sm rounded-xl focus:ring-1 focus:ring-brand-accent focus:border-brand-accent block w-full pl-10 p-2.5 outline-none transition-all placeholder-brand-dark5" placeholder="Filtrar registro para exclusão...">
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap relative">
                    <thead class="bg-brand-dark2/80 text-brand-dark5 text-[11px] uppercase tracking-widest font-semibold border-b border-brand-dark4/60 sticky top-0 z-10 backdrop-blur-md">
                        <tr>
                            ...
                            <th class="py-4 px-6">Deletar</th>
                        </tr>
                    </thead>
                    
                    <tbody id="corpoTabela" class="text-sm text-gray-300 divide-y divide-brand-dark4/30">
                        <?php if (count($cordenacoes) > 0): ?>
                            
                            <?php foreach ($cordenacoes as $cordenacao): ?>
                                <?php $nomeCordenacao = $cordenacao['nome']; ?>

                                <tr class="transition-colors duration-200 group hover:bg-red-900/10 hover:border-l-red-500 border-l-4 border-l-transparent linha-registro">
                                    <td class="py-4 px-6">
                                        <span class="text-brand-dark5 font-mono text-xs">#<?php echo htmlspecialchars($cordenacao['id']); ?></span>
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full bg-brand-dark3 border border-brand-dark4/60 flex items-center justify-center text-[11px] font-bold text-white uppercase shadow-inner">
                                                <?php echo mb_substr(htmlspecialchars($nomeCordenacao), 0, 1, 'UTF-8'); ?>
                                            </div>
                                            <span class="font-medium text-gray-200">
                                                <?php echo htmlspecialchars($nomeCordenacao); ?>
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center text-gray-400 gap-2">
                                            ...
                                            <span><?php echo htmlspecialchars($cordenacao['email']); ?></span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center text-gray-500 gap-2">
                                            ...
                                            <span class="font-mono text-xs opacity-80">
                                                <?php echo htmlspecialchars($cordenacao['senha']); ?>
                                            </span>
                                        </div>
                                    </td>
                                    
                                    <td class="py-4 px-6">
                                        <a href="api/deletar/delete.php?table=cordenacao&id=<?php echo $cordenacao['id']; ?>" 
                                           onclick="return confirm('Tem certeza que deseja excluir o coordenador <?php echo htmlspecialchars($nomeCordenacao); ?>? Esta ação não pode ser desfeita.');"
                            ...
                            <tr>
                                <td colspan="5" class="py-12 text-center text-brand-dark5">
                                    <div class="flex flex-col items-center justify-center gap-3">
                                        <i data-lucide="shield-off" class="w-10 h-10 opacity-40"></i>
                                        <p class="text-base font-light">Nenhum coordenador cadastrado no momento.</p>
                                    </div>
                                </td>
                            </tr>
                        <?php endif; ?>
            ...
            <div class="mt-6 flex items-center justify-between text-xs text-brand-dark5">
                <p>Total de coordenadores no sistema: <span class="font-bold text-white"><?php echo count($cordenacoes); ?></span><span id="contadorFiltro" class="font-medium text-brand-accent ml-1"></span></p>
            </div>
