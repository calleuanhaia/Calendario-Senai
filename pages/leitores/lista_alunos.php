<?php
    ...
    // Função de suporte para os badges coloridos
...
<head>
    ...
    <title>SENAI Integra - Exclusão de Alunos</title>
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-brand-dark2 rounded-xl flex items-center justify-center border border-brand-accent/30 shadow-[0_0_15px_rgba(0,92,169,0.2)]">
                        <i data-lucide="user-minus" class="text-brand-accent w-6 h-6"></i>
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold text-white mb-0.5">Remoção de Alunos</h1>
                        <p class="text-sm text-brand-dark5 font-light">Gerencie e exclua os registros de alunos do sistema.</p>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap relative">
                    <thead class="bg-brand-dark2/80 text-brand-dark5 text-[11px] uppercase tracking-widest font-semibold border-b border-brand-dark4/60 sticky top-0 z-10 backdrop-blur-md">
                        <tr>
                            ...
                            <th class="py-4 px-6">Matricula</th>
                    ...
                    <tbody id="corpoTabela" class="text-sm text-gray-300 divide-y divide-brand-dark4/30">
                        <?php if (count($alunos) > 0): ?>
                            ...
                            <?php foreach ($alunos as $aluno): ?>
                                ...
                                <tr class="transition-colors duration-200 group hover:bg-red-900/10 hover:border-l-red-500 border-l-4 border-l-transparent linha-aluno">
                                    <td class="py-4 px-6">
                                    ...
                                    <td class="py-4 px-6">
                                        <a href="api/deletar/delete.php?table=alunos&id=<?php echo $aluno['id']; ?>" 
                                           onclick="return confirm('Tem certeza que deseja excluir o aluno <?php echo htmlspecialchars($nomeAluno); ?>? Esta ação não pode ser desfeita.');"
