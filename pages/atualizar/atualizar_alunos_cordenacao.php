...
<div class="max-w-2xl mx-auto w-full bg-brand-dark1/60 backdrop-blur-md border border-brand-dark4/40 rounded-2xl p-8 shadow-[0_10px_30px_rgba(0,0,0,0.3)] mt-8">
    ...
    <div class="flex items-center gap-4 mb-8 border-b border-brand-dark4/40 pb-6">
        ...
        <div>
            <h2 class="text-2xl font-bold text-white">Editar Registro do Aluno</h2>
            <p class="text-sm text-brand-dark5 mt-1">Atualize as informações cadastrais e credenciais de acesso.</p>
        </div>
    </div>

    <form action="api/atualizar/atualiza_alunos_cordenacao.php" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="text" name="nome" required 
                    ...
                    value="<?php echo isset($aluno['nome']) ? htmlspecialchars($aluno['nome']) : ''; ?>"
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="email" name="email" required 
                    class="w-full pl-12 pr-4 py-3 bg-brand-dark2/50 border border-brand-dark4/60 rounded-xl text-sm text-white placeholder-brand-dark5 focus:outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent transition-all duration-300"
                    value="<?php echo isset($aluno['email']) ? htmlspecialchars($aluno['email']) : ''; ?>"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-xs font-semibold text-brand-dark5 uppercase tracking-wider mb-2">Matrícula / RA</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <i data-lucide="badge" class="w-5 h-5 text-brand-dark5"></i>
                    </div>
                    <input type="text" name="login-id" required 
                        class="w-full pl-12 pr-4 py-3 bg-brand-dark2/50 border border-brand-dark4/60 rounded-xl text-sm text-white placeholder-brand-dark5 focus:outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent transition-all duration-300"
                        value="<?php echo isset($aluno['matricula']) ? htmlspecialchars($aluno['matricula']) : ''; ?>"
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="password" name="password" 
                        class="w-full pl-12 pr-4 py-3 bg-brand-dark2/50 border border-brand-dark4/60 rounded-xl text-sm text-white placeholder-brand-dark5 focus:outline-none focus:border-brand-accent focus:ring-1 focus:ring-brand-accent transition-all duration-300"
                        value="<?php echo isset($aluno['senha']) ? htmlspecialchars($aluno['senha']) : ''; ?>"
                        placeholder="••••••••">
                </div>
            </div>
        </div>
