<?php
    ...
    if (isset($_SESSION['usuario_email'])) {
        ...
        $sql = "SELECT * FROM cordenacao WHERE email = :email";
...
<div class="max-w-2xl mx-auto w-full bg-brand-dark1/60 backdrop-blur-md border border-brand-dark4/40 rounded-2xl p-8 shadow-[0_10px_30px_rgba(0,0,0,0.3)] mt-8">
    ...
    <div class="flex items-center gap-4 mb-8 border-b border-brand-dark4/40 pb-6">
        ...
        <div>
            <h2 class="text-2xl font-bold text-white">Editar Coordenação</h2>
            <p class="text-sm text-brand-dark5 mt-1">Atualize as suas informações cadastrais e senha de acesso.</p>
        </div>
    </div>

    <form action="api/atualizar/atualiza_cordenacao.php" method="POST" class="space-y-6">
